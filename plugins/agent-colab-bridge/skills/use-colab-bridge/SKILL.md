...
description: "Use when the user asks Codex to run work on Google Colab, use a Colab GPU, train/evaluate models remotely, inspect Colab GPU status, or manage long-running Colab jobs through the Agent Colab Bridge MCP tools."
---

# Use Colab Bridge

Use the Agent Colab Bridge tools only for Colab runtimes the user controls. The bridge gives Codex remote code execution inside the Colab VM, so treat shell, Python, file-write, job-start, and interrupt tools as dangerous.
