...
`agent-colab-bridge` connects MCP-capable coding agents to a Google Colab
runtime through a local MCP server, a Cloudflare Worker/Durable Object, and an
outbound runner process in Colab. It includes first-class setup paths for Codex,
...
`plugins/agent-colab-bridge`. The Codex app installs that small payload, not the
...
```bash
npm run package:plugin
codex plugin marketplace add /absolute/path/to/agent-colab-bridge
codex plugin add agent-colab-bridge@agent-colab-bridge
```

For a published GitHub repo:

```bash
codex plugin marketplace add https://github.com/Aqu1bp/agent-colab-bridge
codex plugin add agent-colab-bridge@agent-colab-bridge
```
...
```bash
npm run doctor -- \
  --config ~/.config/agent-colab-bridge/config.json \
  --require-network
```
...
npm run setup:bridge -- --config ~/.config/agent-colab-bridge/config.json
```

Check local prerequisites and config shape:

```bash
npm run doctor -- --config ~/.config/agent-colab-bridge/config.json
npm run doctor -- --config ~/.config/agent-colab-bridge/config.json --skip-network
```
...
- See `agent-colab-bridge-spec.md` for the implementation spec.
