...
if (!existsSync(serverPath) && !existsSync(sourcePath)) {
  process.stderr.write(
    "agent-colab-bridge MCP server build output is missing from this plugin install. Reinstall the plugin or run `npm run package:plugin` from a source checkout.\n",
  );
  process.exit(1);
}
...
if (!existsSync(serverPath) && !existsSync(tscPath)) {
  ...
  if (result.status !== 0) {
    ...
    process.stderr.write(
      `agent-colab-bridge MCP server dependencies are not installed, and \`npm ${installArgs.join(" ")}\` failed. Run \`npm install\` in the source checkout.\n`,
    );
...
if (!existsSync(serverPath)) {
  ...
  if (result.status !== 0) {
    forwardStartupOutput(result.stdout, result.stderr);
    process.stderr.write(
      "agent-colab-bridge MCP server is not built, and `npm run build` failed. Run `npm install && npm run build` in the plugin checkout.\n",
    );
    process.exit(result.status ?? 1);
  }
}
