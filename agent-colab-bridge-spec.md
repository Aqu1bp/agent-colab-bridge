# agent-colab-bridge Implementation Spec
...
that bootstrap step, the MCP bridge controls the already-live runtime.
...
- `google-colab-cli` can manage Colab notebooks/sessions from the local machine and supports commands such as `new`, `install`, `upload`, `exec`, `download`, `status`, `sessions`, and `url`. Use it for runtime bootstrap and artifact movement rather than making Cloudflare an artifact transport. Source: https://github.com/googlecolab/google-colab-cli
...
2. Create a bridge session from the local machine. The setup command stores the controller token in local config and returns the one-time runner token for bootstrap.
3. Run the local bootstrap script, which shells out through `uvx --from google-colab-cli colab ...`.
4. The bootstrap script creates or reuses a named Colab session, requests GPU/T4 where possible, installs `websockets`, creates the project root, uploads the Python runner, uploads a temporary runner config file, and starts the runner with Colab-side environment variables:
...
5. The bootstrap output prints:

   ```text
   SESSION_ID=...
   BRIDGE_URL=...
   RUNNER_STATUS=start_requested or connected
   ```

   The bootstrap output must not print controller or runner tokens. The generated start code must not embed runner token values; it should read them from the temporary runner config file and delete that file after loading it.

6. Configure the local MCP adapter with the Worker URL, `session_id`, and locally stored controller token.
7. Codex uses MCP tools to check status, upload small files, start jobs, tail logs, and interrupt jobs.
8. If `google-colab-cli` cannot authenticate or provision the runtime, fall back to manually opening a Colab notebook, installing `websockets`, uploading the runner, setting the same Colab-side environment variables, and running `/content/project/colab_runner.py`.
9. Training checkpoints and larger artifacts are saved outside the bridge path or moved with `google-colab-cli download`/`upload`, e.g. Google Drive, Hugging Face Hub, GCS, R2, or GitHub Releases.
...
- Bootstrap configuration generated by the local `google-colab-cli` script
...
- Local `scripts/bootstrap-colab.mjs` wrapper around `uvx --from google-colab-cli colab ...`
...
- GitHub for code
- `google-colab-cli upload` / `download` for explicit local-to-Colab file movement
...
- Local `google-colab-cli` bootstrap script with dry-run-safe tests for command planning and token redaction.
...
- Bootstrap can create or reuse a named Colab session, request T4/GPU, install `websockets`, upload the runner, and start it without printing tokens or embedding the runner token in the executed start source.
- Manual notebook bootstrap remains documented as a fallback.
