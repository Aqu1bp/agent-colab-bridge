...
If `colab_status` says the runner is offline, ask the user to run one of:

```bash
npm run setup:all -- --bootstrap
```

or, if the runner was already started by this repo's bootstrap:
